...
void Fun4All_Prdf_Combiner(int nEvents = 0,
			   int firstevent = 0,
			   int lastevent = 999,
			   const string &outfile = "outfile.root",
...
//  clkchk->Verbosity(3);
  ...
  Fun4AllOutputManager *out = new Fun4AllDstOutputManager("TriggerOut", outfile);
  //  out->DropPacket(21102);
//    out->UseFileRule();
//    out->SetNEvents(1000000);                       // number of events per output file 
