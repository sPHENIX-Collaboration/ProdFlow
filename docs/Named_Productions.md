...
```bash
source /sphenix/u/sphnxpro/mainkolja/sphenixprod/this_sphenixprod.sh && \
production_control.py --steerfile /sphenix/u/sphnxpro/Production2026/run3oo_calo_prod1/run3oo_calo_prod1/autopilot_run3oo_calo_physics_prod1_2025p009.yaml
```
Console output in cron jobs spams emails to whoever is at the receiving end (Chris), so in practice we need to redirect the output. It gets logged automatically anyway.
